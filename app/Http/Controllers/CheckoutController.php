...
class CheckoutController extends Controller
{
    ...
    public function index()
    {
        if ($this->cartService->isEmpty()) {
            return redirect()->route('cart.index')->with('error', __('messages.checkout.cart_empty'));
        }
        ...
        try {
            ...
            foreach ($cart as $cartKey => $item) {
                ...
                if (isset($item['type']) && $item['type'] === 'rental') {
                    ...
                    if (!$rentalItem) {
                        DB::rollBack();
                        return redirect()->back()->with('error', __('messages.checkout.rental_not_found', ['name' => $item['name']]));
                    }
                    ...
                    if (!$rentalItem->isAvailable($startDate, $endDate, $item['quantity'])) {
                        DB::rollBack();
                        return redirect()->back()->with('error', __('messages.checkout.rental_unavailable', ['name' => $item['name']]));
                    }
                    ...
                    if (!$product) {
                        DB::rollBack();
                        return redirect()->back()->with('error', __('messages.checkout.product_not_found', ['name' => $item['name']]));
                    }
                    ...
                    OrderItem::create([
                        ...
                        'quantity' => $item['quantity'],
                        'subtotal' => $item['price'] * $item['quantity'],
                        'variant_attributes' => !empty($attributes) ? $attributes : null,
                    ]);
            ...
            DB::rollBack();
            return redirect()->back()->with('error', __('messages.checkout.error'));
        }
    }
}
