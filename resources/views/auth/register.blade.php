<x-guest-layout>
    <div class="bg-white rounded-2xl shadow-soft p-8 border border-neutral-200">
        ...
        <!-- Email déjà utilisé → suggestion de connexion -->
        @if (session('email_already_exists'))
            <div class="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-3">
                ...
                <div>
                    <p class="text-sm font-semibold text-amber-800">{{ __('messages.auth.email_already_registered') }}</p>
                    <a href="{{ route('login') }}" class="text-sm text-primary-600 hover:text-primary-700 font-semibold underline mt-1 inline-block">
                        {{ __('messages.auth.sign_in_instead') }}
                    </a>
                </div>
            </div>
        @endif
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email') }}" required
                        class="input-field pl-12 @if(session('email_already_exists')) border-amber-400 focus:ring-amber-400 @endif" placeholder="{{ __('auth.register.email_placeholder') }}">
                </div>
            </div>
