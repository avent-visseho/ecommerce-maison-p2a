...
@section('title', 'Modifier Article de Location')
@section('page-title', 'Modifier Article de Location')
...
@section('content')
    <form action="{{ route('admin.rental-items.update', $rentalItem) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-2xl font-bold text-neutral-900">Modifier Article de Location</h2>
                <p class="text-sm text-neutral-400 mt-1">{{ $rentalItem->name }}</p>
            </div>
                    ...
                    <div class="card-body space-y-4">
                        <div>
                            ...
                            <input type="text" id="name" name="name"
                                value="{{ old('name', $rentalItem->name) }}" class="input-field" required>
                            @error('name')
                        ...
                        <div>
                            ...
                            <input type="text" id="sku" name="sku" value="{{ old('sku', $rentalItem->sku) }}"
                                class="input-field">
                            @error('sku')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3"
                                class="input-field">{{ old('description', $rentalItem->description) }}</textarea>
                            @error('description')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input id="long_description" type="hidden" name="long_description"
                                value="{{ old('long_description', $rentalItem->long_description) }}">
                    ...
                    <div class="card-body space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="number" id="daily_rate" name="daily_rate"
                                    value="{{ old('daily_rate', $rentalItem->daily_rate) }}" step="0.01" min="0"
                            ...
                            <div>
                                ...
                                <input type="number" id="weekly_rate" name="weekly_rate"
                                    value="{{ old('weekly_rate', $rentalItem->weekly_rate) }}" step="0.01" min="0"
                                    class="input-field">
                                @error('weekly_rate')
                                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" id="monthly_rate" name="monthly_rate"
                                    value="{{ old('monthly_rate', $rentalItem->monthly_rate) }}" step="0.01" min="0"
                                    class="input-field">
                                @error('monthly_rate')
                                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                                @enderror
                        ...
                        <div>
                            ...
                            <input type="number" id="deposit_amount" name="deposit_amount"
                                value="{{ old('deposit_amount', $rentalItem->deposit_amount) }}" step="0.01" min="0"
                                class="input-field" required>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" id="min_rental_days" name="min_rental_days"
                                    value="{{ old('min_rental_days', $rentalItem->min_rental_days) }}" min="1"
                                    class="input-field">
                            ...
                            <div>
                                ...
                                <input type="number" id="max_rental_days" name="max_rental_days"
                                    value="{{ old('max_rental_days', $rentalItem->max_rental_days) }}" min="1"
                                    class="input-field">
                ...
                <div class="card">
                    ...
                    <div class="card-body space-y-4">
                        ...
                        @if ($rentalItem->main_image)
                            <div>
                                <label class="label">Image Principale Actuelle</label>
                                <div class="flex items-center space-x-4">
                                    <img src="{{ asset('storage/' . $rentalItem->main_image) }}"
                                        alt="{{ $rentalItem->name }}" class="w-32 h-32 object-cover rounded-lg">
                                    <div>
                                        <p class="text-sm text-neutral-600">Télécharger une nouvelle image pour remplacer
                                            celle-ci</p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div>
                            <label for="main_image" class="label">
                                {{ $rentalItem->main_image ? 'Nouvelle Image Principale' : 'Image Principale' }}
                            </label>
                        ...
                        @if ($rentalItem->images && count($rentalItem->images) > 0)
                            <div>
                                <label class="label">Images Supplémentaires Actuelles</label>
                                ...
                                <p class="text-sm text-neutral-400 mt-2">Télécharger de nouvelles images pour remplacer
                                    celles-ci</p>
                            </div>
                        @endif
                        ...
                        <div>
                            <label for="images" class="label">
                                {{ $rentalItem->images ? 'Nouvelles Images Supplémentaires' : 'Images Supplémentaires' }}
                            </label>
                    ...
                    <div class="card-body space-y-4">
                            ...
                            <select id="rental_category_id" name="rental_category_id" class="input-field" required>
                                ...
                                @foreach (\App\Models\RentalCategory::active()->orderBy('name')->get() as $category)
                                    <option value="{{ $category->id }}"
                                        {{ old('rental_category_id', $rentalItem->rental_category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" id="quantity" name="quantity"
                                value="{{ old('quantity', $rentalItem->quantity) }}" min="0" class="input-field"
                                required>
                            @error('quantity')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="card-body space-y-4">
                        <div class="flex items-center justify-between">
                            ...
                            <input type="checkbox" id="is_active" name="is_active" value="1"
                                {{ old('is_active', $rentalItem->is_active) ? 'checked' : '' }} class="toggle">
                        </div>

                        <div class="flex items-center justify-between">
                            <label for="is_featured" class="label mb-0">En Vedette</label>
                            <input type="checkbox" id="is_featured" name="is_featured" value="1"
                                {{ old('is_featured', $rentalItem->is_featured) ? 'checked' : '' }} class="toggle">
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body space-y-3">
                        <div class="flex justify-between text-sm">
                            <span class="text-neutral-600">Vues</span>
                            <span class="font-semibold">{{ $rentalItem->views }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-neutral-600">Réservations</span>
                            <span class="font-semibold">{{ $rentalItem->reservations()->count() }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-neutral-600">Créé le</span>
                            <span class="font-semibold">{{ $rentalItem->created_at->format('d/m/Y') }}</span>
                        </div>
                    </div>
                </div>
            </div>
