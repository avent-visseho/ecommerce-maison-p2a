...
@section('content')
            ...
            <div class="flex-1">
                ...
                <p class="text-sm text-neutral-400 mt-1">
                    ...
                    @elseif($attribute->type === 'checkbox')
                        <span class="badge" style="background-color: #f0fdf4; color: #15803d;">Cases à cocher</span>
                    @else
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-neutral-200">
                        ...
                        <tbody class="bg-white divide-y divide-neutral-200">
                                ...
                                <tr class="hover:bg-neutral-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <button onclick="editValue({{ $value->id }}, '{{ $value->value }}', '{{ $value->code }}', '{{ $value->color_hex }}', {{ $value->price ?? 'null' }}, {{ $value->sort_order }}, {{ $value->is_active ? 'true' : 'false' }})"
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ $attribute->type === 'color' || $attribute->type === 'checkbox' ? '7' : '6' }}" class="px-6 py-12 text-center">
            ...
            <form action="{{ route('admin.attributes.values.store', $attribute) }}" method="POST" class="space-y-4">
                ...
                @if($attribute->type === 'checkbox')
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="price" class="input-field" placeholder="0" min="0" step="0.01">
                            <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                                <span class="text-neutral-400 text-sm">€</span>
                            </div>
                        </div>
                        <p class="text-xs text-neutral-400 mt-1">Prix ajouté au total quand cette option est cochée par le client</p>
                        @error('price')
                            <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <form id="editValueForm" method="POST" class="space-y-4">
                ...
                @if($attribute->type === 'checkbox')
                    <div>
                        <label class="label">Prix unitaire (€)</label>
                        <div class="relative">
                            <input type="number" name="price" id="edit_price" class="input-field" placeholder="0" min="0" step="0.01">
                            <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                                <span class="text-neutral-400 text-sm">€</span>
                            </div>
                        </div>
    ...
    @push('scripts')
        <script>
            ...
            function editValue(id, value, code, colorHex, price, sortOrder, isActive) {
