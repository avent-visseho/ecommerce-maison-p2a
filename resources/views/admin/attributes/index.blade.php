            ...
            <form action="{{ route('admin.attributes.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="type" class="input-field" required>
                        ...
                        <option value="checkbox">Cases à cocher (sélection multiple avec prix)</option>
                    </select>
                    <p class="text-xs text-neutral-400 mt-1">Le type "Cases à cocher" permet au client de sélectionner plusieurs options (ex: parfums). Chaque option peut avoir un prix.</p>
                    @error('type')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
